...
export const AuthProvider = ({ children }) => {
  ...
  // Initialize theme & users from localStorage
  useEffect(() => {
    ...
    // 2. Users initialization & seeding demo account
    let storedUsers = [];
    try {
      ...
      console.error('Error reading lumina_users from localStorage:', err);
    }

    // Ensure demo account exists in users DB
    ...
    try {
      ...
      console.error('Error reading lumina_user session:', err);
    }
  }, []);

  // Theme toggle with document attribute and localStorage sync
  const toggleTheme = () => {
